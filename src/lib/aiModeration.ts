...
이 응답이 조직의 변화나 발전 방향에 대한 진지한 의견(짧아도 괜찮음)인지 판단해 주세요.
아래 중 하나라도 해당하면 OFF_TOPIC입니다:
- 질문/주제와 전혀 관련 없는 장난스럽거나 무의미한 내용
- 테스트/오타/키보드 아무거나 입력한 듯한 내용
- 실제 자기 의견 없이, 위 질문 문구 자체를 그대로(또는 거의 그대로) 베껴 쓴 내용
- 조직 변화와 관련된 단어를 쓰고 있지만, 실질적인 본인 생각이나 제안이 전혀 없이 막연한 말만 반복하는 내용
그 외 짧더라도 본인의 생각이나 제안이 담긴 응답은 RELEVANT입니다.
다른 설명 없이 아래 형식으로만 답하세요:
RELEVANT 또는 OFF_TOPIC
그다음 줄에 이유를 ${MAX_REASON_LENGTH}자 이내의 아주 짧은 구절로만 적어주세요. 완전한 문장이나 마침표 없이, 라벨처럼 짧게 (예: "주제와 무관한 잡담", "질문을 그대로 베낌").`;
