...
접수(사진 업로드)와 실시간 추첨(슬롯머신 릴 애니메이션 + 캐릭터 영상)으로 구성된 사내 이벤트 웹앱입니다.

## 구성

- `/entry` — 참여자용 접수 페이지 (소속, 이름, 사진 업로드) — 참여자에게 공유하는 링크
- `/draw` — 관리자용 추첨 페이지 (비밀번호로 보호) — 행사 진행자가 화면에 띄우는 페이지
- `/admin-login` — `/draw` 접근을 위한 비밀번호 입력 페이지

데이터(참여자 정보, 사진)는 [Supabase](https://supabase.com)의 Postgres 테이블과 Storage 버킷에 저장되고, 앱은 [Vercel](https://vercel.com)에 배포합니다.

## 1. Supabase 설정

1. [supabase.com](https://supabase.com)에서 새 프로젝트를 만듭니다.
2. 프로젝트의 **SQL Editor**에서 [`supabase/schema.sql`](./supabase/schema.sql) 내용을 실행해 `entries` 테이블을 만듭니다.
3. **Storage**에서 `entry-photos`라는 이름의 **Public bucket**을 만듭니다.
4. **Project Settings > API**에서 다음 값을 확인해 둡니다.
   - `Project URL` → `NEXT_PUBLIC_SUPABASE_URL`
   - `service_role` 키 (⚠️ 비공개로 관리) → `SUPABASE_SERVICE_ROLE_KEY`
...
| `SUPABASE_SERVICE_ROLE_KEY` | Supabase service_role 키 (서버에서만 사용, 절대 클라이언트에 노출되지 않음) |
...
[http://localhost:3000/entry](http://localhost:3000/entry) — 접수 페이지
...
3. Deploy 하면 `https://<프로젝트명>.vercel.app/entry` 링크를 참여자에게 공유할 수 있습니다.

## 진행 순서 (행사 당일)

1. 참여자들에게 `/entry` 링크를 공유해 접수를 받습니다.
2. 진행자는 `/admin-login`에서 비밀번호를 입력해 `/draw`에 접속합니다.
3. **추첨 시작** 버튼을 누르면:
   - 인트로 영상 재생 → 슬롯머신 릴이 접수된 사진들을 빠르게 돌다가 서버에서 무작위로 선택된 당첨자에서 멈춤 → 당첨자 소속/이름/사진 표시 + 축하 영상 재생
