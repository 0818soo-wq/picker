...
- `/entry` — 접수 페이지 — 지역단장/파트장 구분 없이 모두에게 공유하는 하나의 링크 (`/entry/leader`, `/entry/staff`, `/br`, `/of`로 접속해도 동일한 화면으로 연결됩니다)
...
1. 지역단장/본사 파트장 구분 없이 모두에게 `/entry` 링크 하나만 공유해 접수를 받습니다. 추첨 대상 여부는 자동으로 판단됩니다.
